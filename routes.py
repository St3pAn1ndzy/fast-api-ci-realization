...
async def get_recipes(session: AsyncSession = Depends(get_session)):
    result = await session.execute(
        select(models.RecipeOrm).order_by(
            desc(models.RecipeOrm.views), asc(models.RecipeOrm.cook_time)
        )
    )
...
async def get_recipe(id_r: int, session: AsyncSession = Depends(get_session)):
